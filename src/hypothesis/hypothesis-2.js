...
export function runHypothesis2(observed = [[], []]) {
	// Combine the two groups into one array and assign a group number (0 or 1) to each value
	const combined = [
		...observed[0].map((value) => ({ value, group: 0 })), // Group 0
		...observed[1].map((value) => ({ value, group: 1 })), // Group 1
	];
	...
	const rankedGroups = [[], []]; // Array to hold the ranked groups
	...
	console.table({
		...
		z, // Z-score
		p: p.toFixed(52), // P-value (rounded for readability)
	});
}
